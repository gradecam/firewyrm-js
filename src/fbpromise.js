...
define([], function() {
    ...
    function makeDeferred() {
        var dfd = new DeferredObject();
        if (arguments.length > 0) {
            dfd.resolve(arguments[1]);
        }
        return dfd;
    }
    makeDeferred.resolved = makeDeferred;
    makeDeferred.rejected = function(err) {
        return makeDeferred().reject(err);
    };
